	...
	public function run()
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			try
			{
				...
				$stmt = "UPDATE phpbb_serversboard SET server_status = 1, server_hostname = '%s', server_map = '%s', server_players = '%d / %d' WHERE server_id = %d";
				...
				if (is_array($players))
				{
					...
					$this->db->sql_query("UPDATE phpbb_serversboard SET server_playerlist = '$players' WHERE server_id = {$row['server_id']}");
				}
			}
			catch (SocketException $e)
			{
				$stmt = "UPDATE phpbb_serversboard SET server_status = 0 WHERE server_id = %d";
